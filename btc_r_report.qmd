...
### Task 1.1 — Prepare series + descriptive stats

```{r task1_series_stats}
Q <- df$Q
Q <- Q[is.finite(Q)]
Q_pos <- Q[Q > 0]
...
### Task 1.2 — Fit + compare distributions
...
### Task 1.3 — Plot 1/4: Histogram + fitted PDFs

```{r task1_plot_hist_pdfs}
...
### Task 1.4 — Plot 2/4: ECDF + fitted CDFs

```{r task1_plot_ecdf_cdfs}
...
### Task 1.5 — Plot 3/4: QQ plot (log(Q) vs Normal)

```{r task1_plot_qq_logq}
logQ <- log(Q_pos)
...
### Task 1.6 — Plot 4/4: Density with log10 x-scale

```{r task1_plot_density_log10}
ggplot(tibble(Q = Q_pos), aes(Q)) +
...
### Task 2.1 — Select dry/wet years

```{r task2_select_years}
...
### Task 2.2 — Annual table (preview)

```{r task2_annual_preview}
annual %>% head(10)
```

### Task 2.3 — Seasonal signature (selected years)

```{r task2_seasonal_signature}
...
### Task 2.4 — Plot 1/4: Time series (faceted)

```{r task2_plot_timeseries_faceted}
...
### Task 2.5 — Plot 2/4: Annual cumulative sum
...
### Task 2.6 — Plot 3/4: Monthly boxplots

```{r task2_plot_monthly_boxplots}
...
### Task 2.7 — Plot 4/4: Monthly mean lines

```{r task2_plot_monthly_means}
...
```{r task3_model_data_fit_metrics}
model_df <- raw %>%
  mutate(
    DATETIME = ymd_hms(DATETIME, quiet = TRUE),
    Q = as.numeric(CLOSE)
...
```{r task3_plot_scatter_pred_vs_actual}
